...
(use-package pretty-hydra
  :config
  ;; fix for pretty-hydra for underscore in hint and bad padding
       ...
       ((char-or-string-p hint)
        (if toggle-p
          ...
          (-let* ((escaped-hint (s-replace "^" "\\^" hint))
                  (width (+ width
                            (- (length escaped-hint) (length hint))
                            (s-count-matches "__" escaped-hint))))
...
(use-package alchemist
  ...
  (define-key alchemist-mode-map (kbd "C-c M-r") nil)
  ...
  ((:title (pretty-hydra-title "Alchemist" 'sucicon "nf-custom-elixir")
   ...
   ;;  )
    ...
    (("x" alchemist-mix "mix") ; Prompt for a specific mix task to run.
     ("c" (lambda() (interactive) (alchemist-mix-execute '("compile"))) "compile") ; Compile the whole elixir project. Prompt for the mix env if the prefix
     ("m c" alchemist-mix-compile "compile with params") ; Compile the whole elixir project. Prompt for the mix env if the prefix
